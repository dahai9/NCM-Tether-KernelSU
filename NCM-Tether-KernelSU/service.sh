...
# Main loop - use Rust uevent listener for efficiency
if [ "$1" = "swap_to_ncm" ]; then
  log "Manual trigger: swapping to NCM..."
  swap_to_ncm
  exit 0
fi

log "Starting Rust uevent listener"
# Run listener in background if needed, or just let it block service.sh
$MODDIR/bin/uevent-listener
